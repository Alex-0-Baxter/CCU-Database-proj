...
AS
	...
	SET @menuItemID = (SELECT menuItemID FROM MenuItems WHERE menuName = @itemName)
	INSERT INTO Items (fk_OrderID, fk_MenuItem, Quantity) VALUES (@orderID, @menuItemID, @quantity)

	SELECT value
	FROM STRING_SPLIT(@Extras, ',');
	UPDATE Extras SET fk_MenuItem = @Extras WHERE fk_MenuItem = @Extras

	DECLARE @price SMALLMONEY
	SET @price = (SELECT Price FROM MenuItems WHERE MenuName = @itemName)

	DECLARE @ItemID INT
	SET @ItemID = (SELECT ItemID FROM Items WHERE fk_OrderID = @orderID)
	
	SELECT @orderID, @ItemID, @menuItemID, @quantity, @itemName, @price
...
AS
	BEGIN
		...
		WHERE menuItemID IN (SELECT DISTINCT menuItemID FROM inserted)
